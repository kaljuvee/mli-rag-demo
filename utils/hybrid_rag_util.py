"""
Hybrid RAG utility for property analysis using real database data with local embeddings.
Uses sentence-transformers for embeddings to avoid OpenAI API limitations.
"""
...
class HybridPropertyVectorizer:
    """Property vectorizer using local sentence-transformers model."""
    
    def __init__(self):
        """Initialize with sentence-transformers model."""
        if not SENTENCE_TRANSFORMERS_AVAILABLE:
            # Create a simple TF-IDF based vectorizer as fallback
            from sklearn.feature_extraction.text import TfidfVectorizer
            self.model = TfidfVectorizer(max_features=384, stop_words='english')
            self.embedding_dim = 384
            self.model_name = "TfidfVectorizer"
            self.is_tfidf = True
        else:
    ...
    def get_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        try:
            if self.is_tfidf:
                # For TF-IDF, we need to fit first
    ...
    def vectorize_query(self, query_text: str) -> np.ndarray:
        ...
        if self.is_tfidf:
            # For TF-IDF, we need to use transform (model should already be fitted)
            embeddings = self.model.transform([query_text]).toarray()
            return embeddings[0]
        else:
...
class HybridRAGUtil:
    """Hybrid RAG utility using real database data with local embeddings."""
    
    def __init__(self):
        """Initialize RAG utility."""
        self.vectorizer = HybridPropertyVectorizer()
...
def create_hybrid_rag_util() -> HybridRAGUtil:
    """Factory function to create HybridRAGUtil instance."""
    return HybridRAGUtil()
